...
import jsbeautifier
import traceback
from pdf_utils import clearScreen, countArrayElements, countNonPrintableChars, escapeRegExpString, getBytesFromFile, hexToString, vtcheck
from pdf_crypto import xor
from js_analysis import analyseJS, isJavascript, unescape
from pdf_core import (MAL_ALL, MAL_BAD_HEAD, MAL_EOBJ, MAL_ESTREAM, MAL_HEAD,
                     MAL_XREF, PDFArray, PDFBody, PDFBool, PDFCrossRefEntry,
                     PDFCrossRefSection, PDFCrossRefSubSection, PDFDictionary,
                     PDFFile, PDFHexString, PDFIndirectObject, PDFName, PDFNull,
                     PDFNum, PDFObject, PDFObjectStream, PDFParser, PDFReference,
                     PDFStream, PDFString, PDFTrailer, get_parser_context,
                     jsContexts, jsVulns, monitorizedActions,
                     monitorizedElements, monitorizedEvents, newLine,
                     spacesChars, vulnsDict)
...
class PDFConsole(cmd.Cmd):
    ...
    def do_set(self, argv):
        ...
        if numArgs == 0:
            ...
            for var in vars:
                varContent = self.printResult(str(self.variables[var][0]))
                if varContent == str(self.variables[var][0]):
                    if varContent != 'None' and not re.match(r'\[.*\]', varContent) and not varContent.isdigit():
    ...
    def do_xor(self, argv):
        ...
        if key is not None:
            key = key.replace('0x', '')
            key = key.replace('\\x', '')
            match = re.match(r'[0-9a-f]{1,2}', key)
        ...
        if objectType == 'bool':
            ...
            for char in objectContent:
                if char in spacesChars + demilimiterChars:
                    return None
            hexNumbers = re.findall(r'#([0-9a-f]{2})', objectContent, re.DOTALL | re.IGNORECASE)
    ...
    def printTreeNode(self, node, nodesInfo, expandedNodes=None, depth=0, recursive=True):
        '''
